...
data['text'] = data['title'] + ' ' + data['breadcrumbs'] + ' ' + data['price']+ ' '+ data['brand'] + ' '+ data['product_details'] + pd.Series(feature_text)
...
def get_top_similar_products(query, num_recommendations=5):
    ...
    query = ' '.join([word for word in word_tokenize(query) if not word in stop_words])
